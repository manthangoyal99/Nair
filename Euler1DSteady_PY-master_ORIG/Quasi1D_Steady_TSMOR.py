...
def calc_grid_distortion_basis(xarr,dMu,ib,nf):
    """
...
def calc_grid_distortion(coeffs,xarr,dMu,ng=1):
    """
    ...
    for ib, cc in enumerate(coeffs):
        grid_distortion += cc*calc_grid_distortion_basis(xarr,dMu,ib,nf)
    return grid_distortion
#enddef calc_grid_distortion


def calc_distorted_grid(coeffs,xarr,dMu,ng=1):
    """
    ...
    xd : Distorted grid (supplied original grid 'xarr' plus the grid distortion
         'c_s')
    """
    return xarr + calc_grid_distortion(coeffs,xarr,dMu,ng=ng)
#enddef calc_distorted_grid


def calc_transported_snap(coeffs,xarr,u,dMu,ng=1):
    """
    ...
    xd = calc_distorted_grid(coeffs,xarr,dMu,ng=ng)
...
def calc_transported_snap_error(coeffs,xarr,uRef,uNbs,dMuNbs,ng=1):
    """
    ...
    for inb, dMu in enumerate(dMuNbs): #Go thru all neighbours
        ...
        # grid 'xarr'
        u0t = calc_transported_snap(coeffs,xarr,uRef,dMu,ng=ng)
...
def calc_grid_distortion_constraint(coeffs,xarr,dMuNbs,ng=1):
    """
...
class Project_TSMOR_Offline(object):
    ...
    def __init__(self,u_db,Mus_db,iRef,iNbs,xarr,ng=1):
        """
    ...
    def solnPost(self,output):  
        ...
        for iNb, dMu in enumerate(self.dMuNbs):
            plt.plot(self.xarr,self.uNbs[iNb][:,iv],'--')
            plt.plot(self.xarr,calc_transported_snap(coeffs,self.xarr, \
                self.uRef[:,iv],dMu))
        plt.plot(self.xarr,self.uRef[:,iv],'k-.')
        plt.title('Transported vs. neighbouring snaps')
        plt.subplot(2,2,4)
        for iNb, dMu in enumerate(self.dMuNbs):
            plt.plot(self.xarr,self.uNbs[iNb][:,iv] \
                - calc_transported_snap(coeffs,self.xarr,self.uRef[:,iv],dMu))
