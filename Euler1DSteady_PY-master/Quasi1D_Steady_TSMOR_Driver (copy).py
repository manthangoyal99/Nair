...
if options.train:
    ...
    #for iRef in range(1):
        ...
        iv = 0
        if (normalised==True):
            project_off=tsmor.Project_TSMOR_Offline(u_db_norm[:,iv,:].reshape(-1,1,nMus),\
                Mus_db_norm,iRef,iNbs,xarr,normalised,u_db_inlet)

        else:
            project_off=tsmor.Project_TSMOR_Offline(u_db[:,iv,:].reshape(-1,1,nMus),\
                Mus_db_norm,iRef,iNbs,xarr,normalised,np.ones((np.shape(u_db[0,:,:]))))
